const { json, cors, getStore } = require("../lib/spots");
...
exports.handler = async (event) => {
  ...
  try {
    const store = getStore("fresque");
    let payload = await store.get(CACHE_KEY, { type: "json" });
    ...
    return json(500, { detail: err.message || "311 fetch failed" });
  }
};
